...
import fetchBookDetail from "@/function/fetch/fetchBookDetail";

type Props = {
  className?: string;
  isbn: number;
} & HTMLAttributes<HTMLDivElement>;
const BookDetail = async ({ className, isbn, ...props }: Readonly<Props>) => {
  const bookData = await fetchBookDetail(isbn);
  ...
  return (
    <div className={`relative flex size-full flex-col px-[var(--client-layout-margin)] ${className || ""}`} {...props}>
      ...
      <BookTitleSection
        ...
        url={`https://search.daum.net/search?w=bookpage&bookId=` + isbn + `&tab=introduction`}
        className="mb-10"
      />
      <BookDetailSection
        author={bookData?.authorList.map((item) => item.name).join(", ") || "저자 이름(정보 미제공)"}
        pulisher={bookData?.publisher.name || "출판사(정보 미제공)"}
        description={bookData?.summary || "책 소개(정보 미제공)"}
        viewMoreUrl={`https://search.daum.net/search?w=bookpage&bookId=` + isbn + `&tab=introduction`}
        className="mb-10"
      />
      <section>
