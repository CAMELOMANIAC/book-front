...
const fetchSchedule = async ({ limit, page, eventFlag, eventType, location, startDate, endDate }: Arguments) => {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/events?${queryParams.toString()}`);
    if (response.ok) {
